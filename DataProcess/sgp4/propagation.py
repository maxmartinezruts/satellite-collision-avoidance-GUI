...
def _dpper(satrec, inclo, init, ep, inclp, nodep, argpp, mp, opsmode):
    ...
    if init == 'n':
       ...
       //  discontinuity 0.2 rad = 11.45916 deg
    ...
    if irez != 0:
        ...
        if irez == 2:
            ...
            if em <= 0.65:
                ...
                g310 = -19.302 + 117.3900 * em - \
                    228.4190 * emsq + 156.5910 * eoc
                g322 = -18.9068 + 109.7927 * em - \
                    214.6334 * emsq + 146.5816 * eoc
                g410 = -41.122 + 242.6940 * em - \
                    471.0940 * emsq + 313.9530 * eoc
                g422 = -146.407 + 841.8800 * em - \
                    1629.014 * emsq + 1083.4350 * eoc
                g520 = -532.114 + 3017.977 * em - \
                    5740.032 * emsq + 3708.2760 * eoc

            else:

                g211 = -72.099 + 331.819 * em - 508.738 * emsq + 266.724 * eoc
                g310 = -346.844 + 1582.851 * em - \
                    2415.925 * emsq + 1246.113 * eoc
                g322 = -342.585 + 1554.908 * em - \
                    2366.899 * emsq + 1215.972 * eoc
                g410 = -1052.797 + 4758.686 * em - \
                    7193.992 * emsq + 3651.957 * eoc
                g422 = -3581.690 + 16178.110 * em - \
                    24462.770 * emsq + 12422.520 * eoc
                if em > 0.715:
                    g520 = -5149.66 + 29936.92 * em - \
                        54087.36 * emsq + 31324.56 * eoc
                else:
                    g520 = 1464.74 - 4664.75 * em + 3763.64 * emsq

            if em < 0.7:

                g533 = -919.22770 + 4988.6100 * em - \
                    9064.7700 * emsq + 5542.21 * eoc
                g521 = -822.71072 + 4568.6173 * em - \
                    8491.4146 * emsq + 5337.524 * eoc
                g532 = -853.66600 + 4690.2500 * em - \
                    8624.7700 * emsq + 5341.4 * eoc

            else:

                g533 = -37995.780 + 161616.52 * em - \
                    229838.20 * emsq + 109377.94 * eoc
                g521 = -51752.104 + 218913.95 * em - \
                    309468.16 * emsq + 146349.42 * eoc
                g532 = -40023.880 + 170470.89 * em - \
                    242699.48 * emsq + 115605.82 * eoc
    ...
    if irez != 0:
        ...
        while iretn == 381:
            ...
            if irez != 2:
                ...
                xndt = del1 * sin(xli - fasx2) + del2 * \
                    sin(2.0 * (xli - fasx4)) + del3 * sin(3.0 * (xli - fasx6))
                xldot = xni + xfact
...
*                   1 - mean elements, ecc >= 1.0 or ecc < -0.001 or a < 0.95er
...
def sgp4init(
    ...
    #  sgp4fix identify constants and allow alternate values
    # tumin, mu, radiusearthkm, xke, j2, j3, j4, j3oj2 = whichconst
    _, _, radiusearthkm, _, j2, _, j4, j3oj2 = whichconst
    ...
    (satrec.no, _, _, ao, satrec.con41, con42, cosio, cosio2, eccsq, omeosq,
     posq, rp, rteosq, sinio, satrec.gsto,) = _initl(
        satn, whichconst, satrec.ecco, epoch, satrec.inclo, satrec.no,
        satrec.method, satrec.operationmode)
    satrec.error = 0

    """
     // sgp4fix remove this check as it is unnecessary
     // the mrt check in sgp4 handles decaying satellite cases even if the
     // starting condition is below the surface of te earth
     //     if (rp < 1.0)
     //       {
    ...
    if omeosq >= 0.0 or satrec.no >= 0.0:
        ...
        if 2*pi / satrec.no >= 225.0:
            ...
            # (snodm, cnodm, sinim, cosim, sinomm, cosomm, day, satrec.e3,
            #  satrec.ee2, em, emsq, gam, satrec.peo, satrec.pgho, satrec.pho,
            #  satrec.pinco, satrec.plo, rtemsq, satrec.se2, satrec.se3,
            #  satrec.sgh2, satrec.sgh3, satrec.sgh4, satrec.sh2, satrec.sh3,
            #  satrec.si2, satrec.si3, satrec.sl2, satrec.sl3, satrec.sl4, s1,
            #  s2, s3, s4, s5, s6, s7, ss1, ss2, ss3, ss4, ss5, ss6, ss7, sz1,
            #  sz2, sz3, sz11, sz12, sz13, sz21, sz22, sz23, sz31, sz32, sz33,
            #  satrec.xgh2, satrec.xgh3, satrec.xgh4, satrec.xh2, satrec.xh3,
            #  satrec.xi2, satrec.xi3, satrec.xl2, satrec.xl3, satrec.xl4, nm,
            #  z1, z2, z3, z11, z12, z13, z21, z22, z23, z31, z32, z33,
            #  satrec.zmol, satrec.zmos) = _dscom(
            (_, _, sinim, cosim, _, _, _, satrec.e3,
             satrec.ee2, em, emsq, _, satrec.peo, satrec.pgho, satrec.pho,
             satrec.pinco, satrec.plo, _, satrec.se2, satrec.se3,
             satrec.sgh2, satrec.sgh3, satrec.sgh4, satrec.sh2, satrec.sh3,
             satrec.si2, satrec.si3, satrec.sl2, satrec.sl3, satrec.sl4, s1,
             s2, s3, s4, s5, _, _, ss1, ss2, ss3, ss4, ss5, _, _, sz1,
             _, sz3, sz11, _, sz13, sz21, _, sz23, sz31, _, sz33,
             satrec.xgh2, satrec.xgh3, satrec.xgh4, satrec.xh2, satrec.xh3,
             satrec.xi2, satrec.xi3, satrec.xl2, satrec.xl3, satrec.xl4, nm,
             z1, _, z3, z11, _, z13, z21, _, z23, z31, _, z33,
             satrec.zmol, satrec.zmos) = _dscom(
                epoch, satrec.ecco, satrec.argpo, tc, satrec.inclo,
                satrec.nodeo, satrec.no, satrec.e3, satrec.ee2,
                satrec.peo, satrec.pgho, satrec.pho, satrec.pinco,
                satrec.plo, satrec.se2, satrec.se3, satrec.sgh2, satrec.sgh3,
                satrec.sgh4, satrec.sh2, satrec.sh3, satrec.si2, satrec.si3,
                satrec.sl2, satrec.sl3, satrec.sl4, satrec.xgh2, satrec.xgh3,
                satrec.xgh4, satrec.xh2, satrec.xh3, satrec.xi2, satrec.xi3,
                satrec.xl2, satrec.xl3, satrec.xl4, satrec.zmol, satrec.zmos)
            (satrec.ecco, satrec.inclo, satrec.nodeo, satrec.argpo, satrec.mo
             ) = _dpper(satrec, inclm, satrec.init, satrec.ecco, satrec.inclo,
            ...
            (em, argpm, inclm, mm, nm, nodem, satrec.irez, satrec.atime,
             ...
             satrec.dmdt, _, satrec.dnodt, satrec.domdt, satrec.del1,
             satrec.del2, satrec.del3, satrec.xfact, satrec.xlamo, satrec.xli,
             satrec.xni) = _dsinit(whichconst, cosim, emsq, satrec.argpo, s1,
                                   s2, s3, s4, s5, sinim, ss1, ss2, ss3, ss4,
                                   ss5, sz1, sz3, sz11, sz13, sz21, sz23, sz31,
                                   sz33, satrec.t, tc, satrec.gsto, satrec.mo,
                                   satrec.mdot, satrec.no, satrec.nodeo,
                                   satrec.nodedot, xpidot, z1, z3, z11, z13,
                                   z21, z23, z31, z33, satrec.ecco, eccsq, em,
                                   argpm, inclm, mm, nm, nodem, satrec.irez,
                                   satrec.atime, satrec.d2201, satrec.d2211,
                                   satrec.d3210, satrec.d3222, satrec.d4410,
                                   satrec.d4422, satrec.d5220, satrec.d5232,
                                   satrec.d5421, satrec.d5433, satrec.dedt,
                                   satrec.didt, satrec.dmdt, satrec.dnodt,
                                   satrec.domdt, satrec.del1, satrec.del2,
                                   satrec.del3, satrec.xfact, satrec.xlamo,
    ...
    """
       ...
       // actually below earth surface if(satrec.error == 0)
     """
    sgp4(satrec, 0.0)
...
def sgp4(satrec, tsince, whichconst=None):
    ...
    #  sgp4fix identify constants and allow alternate values
    # tumin, mu, radiusearthkm, xke, j2, j3, j4, j3oj2 = whichconst
    _, _, radiusearthkm, xke, j2, _, _, j3oj2 = whichconst
    ...
    if satrec.method == 'd':
        ...
        (_, em, argpm, inclm, _, mm, _, nodem, _, nm) = _dspace(
            satrec.irez,
