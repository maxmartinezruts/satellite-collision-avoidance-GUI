...
*  variables and initializes the sgp4 variables. several intermediate
*  variables and quantities are determined. note that the result is a
*  structure so multiple satellites can be processed simultaneously without
*  having to reinitialize. The verification mode is an important option that
*  permits quick checks of any changes to the underlying technical theory.
*  This option works using a modified tle file in which the start, stop, and
*  delta time values are included at the end of the second line of data. This
*  only works with the verification mode. the catalog mode simply propagates
*  from -1440 to 1440 min from epoch and is useful when performing entire
*  catalog runs.
*
...
def twoline2rv_sat(i_code, i_year, d_doy, d_ndot, d_nddot, i_nexp,
                   d_bstar_base, i_ibexp,
                   d_inclo, d_nodeo, d_ecco, d_argpo, d_mo, d_no,
                   whichconst, afspc_mode=False):
    """Return a Satellite imported from two lines of TLE data.
    ...
    try:
        # assert line.startswith('1 ')
        satrec.satnum = i_code  # int(line[2:7])
        # classification = line[7] or 'U'
        # assert line[8] == ' '
        # intldesg = line[9:17]
        two_digit_year = i_year  # int(line[18:20])
        # assert line[23] == '.'
        satrec.epochdays = d_doy  # float(line[20:32])
        # assert line[32] == ' '
        # assert line[34] == '.'
        satrec.ndot = d_ndot  # float(line[33:43])
        # assert line[43] == ' '
        satrec.nddot = d_nddot  # float(line[44] + '.' + line[45:50])
        nexp = i_nexp  # int(line[50:52])
        # assert line[52] == ' '
        satrec.bstar = d_bstar_base  # float(line[53] + '.' + line[54:59])
        ibexp = i_ibexp    # int(line[59:61])
        # assert line[61] == ' '
        # assert line[63] == ' '
        # numb = int(line[62])
        # elnum = int(line[64:68])
    ...
    try:
        ...
        # assert line[7] == ' '
        # assert line[11] == '.'
        satrec.inclo = d_inclo  # float(line[8:16])
        # assert line[16] == ' '
        # assert line[20] == '.'
        satrec.nodeo = d_nodeo  # float(line[17:25])
        # assert line[25] == ' '
        satrec.ecco = d_ecco  # float('0.' + line[26:33].replace(' ', '0'))
        # assert line[33] == ' '
        # assert line[37] == '.'
        satrec.argpo = d_argpo  # float(line[34:42])
        # assert line[42] == ' '
        # assert line[46] == '.'
        satrec.mo = d_mo  # float(line[43:51])
        # assert line[51] == ' '
        satrec.no = d_no  # float(line[52:63])
        # revnum = line[63:68]
    except (AssertionError, IndexError, ValueError):
        raise ValueError('ERROR')
